...
public class NurseController {
    ...
    @GetMapping("/sign-in")
    public ResponseEntity<? super GetSignInResponseDto> getSignIn(@AuthenticationPrincipal String userId) {
        ResponseEntity<? super GetSignInResponseDto> response = nurseService.getSignIn(userId);
        return response;
    }

    @GetMapping("/{userId}")
    public ResponseEntity<? super GetNurseResponseDto> getNurse(@PathVariable String userId) {
        ResponseEntity<? super GetNurseResponseDto> response = nurseService.getNurse(userId);
        return response;
    }

    @PatchMapping(value = {"", "/"})
    public ResponseEntity<ResponseDto> patchNurse(@RequestBody @Valid PatchNurseRequestDto requestBody, @AuthenticationPrincipal String userId) {
        ResponseEntity<ResponseDto> response = nurseService.patchNurse(requestBody, userId);
        return response;
    }

    @GetMapping("/{nurseId}/customers")
    public ResponseEntity<? super GetChargedCustomerResponseDto>  getChargedCustomer(@PathVariable String nurseId) {
